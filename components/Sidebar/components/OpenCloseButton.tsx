  ...
  return (
    <>
      <button
        ...
        } z-50 h-7 w-7 hover:text-gray-400 dark:text-white dark:hover:text-gray-300 sm:top-6 sm:${
          side === 'right' ? 'right-[280px]' : 'left-[280px]'
        } sm:h-8 sm:w-8 sm:text-neutral-700`}
        onClick={onClick}
      >
        {side === 'right' ? <IconLayoutSidebarLeftExpand /> : <IconLayoutSidebarRightExpand />}
      </button>
      <div
        onClick={onClick}
...
export const OpenSidebarButton = ({ onClick, side }: Props) => {
  return (
    <button
      ...
      onClick={onClick}
    >
      {side === 'right' ? <IconLayoutSidebarRightExpand /> : <IconLayoutSidebarLeftExpand />}
    </button>
  );
};
