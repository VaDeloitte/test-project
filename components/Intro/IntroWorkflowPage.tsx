...
const IntroWorkflowPage: React.FC<HeaderProps> = ({ workflowsDataInitial, workflowsData, setWorkflowsDataSearch, handleActivate, style, showIcon = true, fullPlaceholder = "Search for agent..." }) => {
    ...
    const handleSearchChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (query.length > 0) {
            ...
            setTimeout(() => { // Simulate delay for heavy computation or data fetching
                const results1 = filterWorkflows(query);
                ...
                setWorkflowsDataSearch(results.length > 0 ? results : [{ title: "" }])
            ...
            setTimeout(() => {
                setWorkflowsDataSearch(workflowsDataInitial)
                setSearchResults([]);
...
export default IntroWorkflowPage;
